...
int main(int argc, char** argv) {
    ...
    SDL_WM_SetCaption("td02 ex01", NULL);

    glClearColor(0.1, 0.1, 0.1, 1.0);
    glClear(GL_COLOR_BUFFER_BIT);

    /* On créé une première primitive par défaut */
    GLenum currentPrimitiveType = GL_LINE_STRIP;
    PrimitiveList primitives = NULL;
    Primitive* prim = allocPrimitive(currentPrimitiveType);
        ...
        while(SDL_PollEvent(&e)) {
            ...
            switch(e.type) {
                ...
                case SDL_MOUSEBUTTONUP:
                {
                    ...
                    // Si on utilise le clic droit en mode GL_LINE_STRIP, alors on termine la boucle et on réinitialise la primitive
                    else if (e.button.button == SDL_BUTTON_RIGHT && currentPrimitiveType == GL_LINE_STRIP) {
