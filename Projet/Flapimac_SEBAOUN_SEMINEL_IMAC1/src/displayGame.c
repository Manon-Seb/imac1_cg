...
float displayGame(int transY_,float speed,float transX,float transY,float transZ,
  ...
  /*** Update coordonnate ***/
      ...
      *projectileEnnemy=updateProjectileEnnemy(&(*projectileEnnemy));
  // ennemyMove is a counter to translate the ennemy on the Y axis and make ennemies shoot projectile
      (*ennemyMove)++;
      if ((*ennemyMove)==120){
        (*ennemy) = updateEnnemy(&(*ennemy),&(*ennemyMove));
        freeProjectile(&(*projectileEnnemy));
        *projectileEnnemy = NULL ;
        // old projectile disappear and ennies shoot new ennemies
        addProjectileEnnemy((*ennemy),&(*projectileEnnemy));
        (*ennemyMove) = 0;
      }
  /*** Collision ***/
      tmp = collision_(*playerBB,obstacle);
      if (tmp == 0){
        // collision between the player and the obstacle has been detected
        return 0;
      }
      tmp = collision_ennemy(*playerBB,*ennemy);
      if (tmp == 0){
        // collision between the player and the ennemy has been detected
        return 0;
      }
      ...
      glColor3ub(0,0,255);
      drawprojectileEnnemy(*projectileEnnemy,speed,textureID);
      glPopMatrix();
      return speed;
}
...
void drawLevel(int level[w][h][3],float speed, float transX, float transY, float transZ,ennemyList ennemy,GLuint textureID[10]){
  ...
  int x=0;
  int y=h/2;
  // Browse every case from the array
  while (j<h){
    i=0;
    while (i<w){
      // if the red color has been detected, we draw a red rect and we texture it
